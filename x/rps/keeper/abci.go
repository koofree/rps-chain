...
func (k Keeper) EndBlocker(ctx context.Context) error {
	...
	rng := collections.NewPrefixUntilPairRange[uint64, uint64](
		uint64(sdkCtx.BlockHeight()),
	)

	if err := k.ActiveGamesQueue.Walk(ctx, rng, func(key collections.Pair[uint64, uint64]) (stop bool, err error) {
